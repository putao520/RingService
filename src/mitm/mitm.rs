...
use crate::mitm::filter_flow::{start_search_paths, start_search_rules, FilterFlowDesc};
...
impl RuleHandler<FilterFlowDesc> for SNIHandler {
    async fn get_rules(&mut self, uri: &Uri) -> Option<FilterFlowDesc> {
        // 查找规则
        let desc = start_search_rules(uri.host().unwrap_or(""));
        match desc {
            Some(d) => start_search_paths(uri.path(), &d),
            None => None,
        }
    }
...
pub async fn mitm_start(mut shutdown_ch: tokio::sync::mpsc::Receiver<()>) -> Result<()> {
    ...
    let (cert_root, key_pair) = ca_system.get_cert_pem().await?;

    let key_pair = KeyPair::from_pem(key_pair.as_str()).expect("Failed to parse private key");
    let ca_cert = CertificateParams::from_ca_cert_pem(cert_root.as_str())
